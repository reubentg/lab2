...
class ParticleFilter():
    ...
    def __init__(self, n_particles, n_viz_particles,
        ...
        # In this implementation, the total number of particles is constant
        self.N_VIZ_PARTICLES = n_viz_particles  # The number of particles to visualize (default=60)

        # create np.array([0,1,2,3,...,997,998,999]), shape (1000,)
        ...
        self.pose_pub = rospy.Publisher(PUBLISH_PREFIX + "/inferred_pose", PoseStamped,
                                        queue_size=1)  # Publishes the expected pose
        self.particle_pub = rospy.Publisher(PUBLISH_PREFIX + "/particles", PoseArray,
                                            queue_size=1)  # Publishes a subsample of the particles
        self.pub_laser = rospy.Publisher(PUBLISH_PREFIX + "/scan", LaserScan,
                                         queue_size=1)  # Publishes the most recent laser scan
        self.pub_odom = rospy.Publisher(PUBLISH_PREFIX + "/odom", Odometry,
...
def main():
    ...
    # The number of particles (default="1000")
    n_particles = int(rospy.get_param("~n_particles"))
    # The number of particles to visualize (default="60")
    n_viz_particles = int(rospy.get_param("~n_viz_particles"))
    # The topic containing motor state information (default="/vesc/sensors/core") - speed
    motor_state_topic = rospy.get_param("~motor_state_topic", "/vesc/sensors/core")
    # The topic containing servo state information (default="/vesc/sensors/servo_position_command") - angle
    servo_state_topic = rospy.get_param("~servo_state_topic", "/vesc/sensors/servo_position_command")
    # The topic containing laser scans (default="/scan")
    scan_topic = rospy.get_param("~scan_topic", "/scan")
    # Step for downsampling laser scans (default="18")
    ...
    steering_angle_to_servo_offset = float(rospy.get_param("/vesc/steering_angle_to_servo_offset", 0.5))
    # Gain conversion param from servo position to steering angle
    steering_angle_to_servo_gain = float(rospy.get_param("/vesc/steering_angle_to_servo_gain", -1.2135))
